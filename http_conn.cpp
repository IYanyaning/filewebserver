    ...
    else if (*(p + 1) == '7')
    {
        ...
        if (!user_name.empty())
        {
            ...
            while (m_string_is_end)
            {
                    ...
                    if (strncmp(line.c_str(), boundary.c_str(), 58) == 0)
                    {
                        ...
                        while (std::getline(iss, line))
                        {
                            ...
                            if (strncmp(line.c_str(), boundary.c_str(), 58) == 0)
                            {
                                ...
                                if (strncmp(line.c_str(), boundary.c_str(), 60) == 0)
                                {
...
//        snprintf( response, sizeof(response), "HTTP/1.1 200 OK\r\nContent-Length: %d\r\nContent-Type: text/plain\r\n\r\n下载文件: http://192.168.1.107/time.txt",strlen("下载文件: http://192.168.1.107/time.txt"));
