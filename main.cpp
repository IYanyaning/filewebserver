...
void cb_func( client_data* user_data){
    ...
    LOG_INFO("------------------client [%s] is closed----------------",inet_ntoa(user_data->address.sin_addr));
    ...
    while(!stop_server){
        ...
        for(int i = 0;i < number; i++){
            ...
            if(sockfd == listenfd){
                ...
                LOG_INFO("------------------client [%s] is coming----------------", inet_ntoa(users[connfd].get_client_data()->address.sin_addr));
                ...
                if(int read_byte = users[sockfd].read()){
                    LOG_INFO("deal with the client(%s)", inet_ntoa(users[sockfd].get_client_data()->address.sin_addr));
                    if(users[sockfd].get_client_data()->timer){
                        time_t cur = time(NULL);
                ...
                if( users[sockfd].write()){
                    LOG_INFO("send data to the client(%s)", inet_ntoa(users[sockfd].get_client_data()->address.sin_addr));
