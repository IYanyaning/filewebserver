...
bool Log::init(const char *file_name, int close_log, int log_buf_size, int split_lines, int max_queue_size){
    ...
    if( max_queue_size >=1){
        ...
        if(!m_log_queue){
//            std::unique_ptr<BlockDeque<std::string>> *newDeque(new BlockDeque<std::string>(max_queue_size));
...
//            std::unique_ptr<std::thread> NewThread(new std::thread([]{Log::get_instance()->async_write_log();}));
